...
const catEmojis = [
  ...
  // reptiles & amphibians (kamu tadi bilang kura, paus boleh, jadi aku keep ya)
  '🐢', '🐍', '🦎', '🐊', '🐸',
...
type FloatingEmoji = { id: number; emoji: string; x: number; y: number; delay: number }
...
export default function Home() {
  ...
  const [catMood, setCatMood] = useState<'happy' | 'excited' | 'lovey'>('happy')
  ...
  // ✅ anti-repeat pool
  const [remainingFacts, setRemainingFacts] = useState<string[]>(() => [...catFunFacts])

  // Generate floating emojis
  useEffect(() => {
    const emojis: FloatingEmoji[] = floatingEmojisList.map((emoji, index) => ({
      id: index,
      emoji,
      x: Math.random() * 100,
      y: Math.random() * 100,
  ...
  const handleWordClick = (index: number) => {
    ...
    const moods: Array<'happy' | 'excited' | 'lovey'> = ['happy', 'excited', 'lovey']
  ...
  const resetGame = () => {
    ...
    setCurrentFunFact('')
    setRemainingFacts([...catFunFacts]) // reset pool juga
  }
  ...
  // ✅ no-repeat funfacts (random tapi ga bakal kepilih lagi sampai pool habis)
  const handleFunFact = () => {
    setRemainingFacts((prev) => {
      const pool = prev.length === 0 ? [...catFunFacts] : prev
    ...
    <div className="min-h-screen flex flex-col bg-gradient-to-br from-pink-100 via-cyan-100 to-purple-100 dark:from-pink-900 dark:via-cyan-900 dark:to-purple-900 overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            <Card className="px-4 py-2 bg-gradient-to-r from-pink-200 to-cyan-200 dark:from-pink-700 dark:to-cyan-700">
              <span className="text-lg md:text-xl font-bold text-pink-800 dark:text-cyan-200">🎮 Score: {score}</span>
            </Card>
          </div>
        </div>
      </header>
      ...
      <main className="relative z-10 flex-1 flex flex-col items-center justify-center p-4 md:p-8 gap-8 md:gap-12">
        ...
        <Card className="max-w-4xl w-full p-6 md:p-8 bg-gradient-to-br from-pink-100 to-cyan-100 dark:from-pink-800 dark:to-cyan-800 shadow-2xl">
          <h2 className="text-2xl md:text-3xl font-bold text-center mb-6 text-pink-800 dark:text-cyan-200">⚡ Cute Mini Games! ⚡</h2>
          ...
          {showFunFact && (
            <Card className="mt-6 p-6 bg-gradient-to-r from-yellow-50 to-orange-50 dark:from-yellow-800 dark:to-orange-800 border-4 border-yellow-300 dark:border-orange-600">
              <p className="text-center text-lg md:text-xl font-bold text-orange-800 dark:text-yellow-200 animate-fade-in">📚 {currentFunFact}</p>
            </Card>
          )}
        </Card>
        ...
        <Card className="max-w-md w-full p-4 md:p-6 bg-gradient-to-r from-pink-100 to-cyan-100 dark:from-pink-700 dark:to-cyan-700">
          <div className="text-center">
            <div className="text-lg md:text-xl font-bold text-pink-800 dark:text-cyan-200 mb-2">Kucing Mode: {catMood.toUpperCase()} ⚡</div>
        ...
        <Card className="max-w-2xl w-full p-6 md:p-8 bg-gradient-to-br from-pink-50 to-cyan-50 dark:from-pink-800 dark:to-cyan-800 shadow-2xl">
          <h2 className="text-2xl md:text-3xl font-bold text-center mb-6 text-pink-800 dark:text-cyan-200">🎨 Generate Random Cute Cat! 🐱⚡</h2>

          {generatedCatImage && (
            <div className="mb-6 relative">
              <img src={generatedCatImage} alt="Generated Cute Cat" className="w-full rounded-2xl shadow-lg animate-fade-in" />
      ...
      <footer className="relative z-10 mt-auto bg-gradient-to-r from-pink-200 via-cyan-200 to-purple-200 dark:from-pink-800 dark:via-cyan-800 dark:to-purple-800 py-6 md:py-8">
        <div className="max-w-7xl mx-auto px-4 text-center">
          <div className="text-lg md:text-2xl font-bold text-pink-800 dark:text-cyan-200 mb-2">CIPA CEOLANG CAT LOVER 🐱⚡</div>
          <p className="text-sm md:text-base text-pink-700 dark:text-cyan-300">Built with ⚡ for CIPA! 🚀</p>
